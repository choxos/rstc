...
if (!exists("causal_forest_stc_analysis")) {
  causal_forest_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols, 
                                         target_population = NULL, num_trees = 2000, seed = 123) {
    if (!requireNamespace("grf", quietly = TRUE)) {
      stop("grf package not available")
    }
    # This will only run if grf is available
    stop("Causal Forest implementation requires grf package - using fallback")
  }
}
...
if (functions_loaded < 8) {
  cat("⚠️ Some methods may not be available in the analysis below.\n")
}
```
