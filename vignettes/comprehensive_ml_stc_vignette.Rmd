...
# Helper function to detect outcome type
detect_outcome_family <- function(y) {
  # Check if binary (0/1 or logical)
  if (all(y %in% c(0, 1)) || is.logical(y)) {
    return("binomial")
  }
  # Check if bounded between 0 and 1 (like proportions)
  if (all(y >= 0 & y <= 1)) {
    return("binomial") 
  }
  ...
  # Default to gaussian for continuous
  return("gaussian")
}
...
if (!exists("t_learner_stc_analysis")) {
  t_learner_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols, 
    ...
    # Detect appropriate family for outcome
    outcome_family <- detect_outcome_family(data[[outcome_col]])
    prediction_type <- if (outcome_family == "binomial") "response" else "response"
    ...
    tryCatch({
      model_1 <- glm(formula_obj, data = treated, family = outcome_family)
      model_0 <- glm(formula_obj, data = control, family = outcome_family)
      
      # Predict for all individuals
      pred_1 <- predict(model_1, newdata = data, type = prediction_type)
      pred_0 <- predict(model_0, newdata = data, type = prediction_type)
      ...
      result <- list(
        ...
        target_predictions = if (!is.null(target_population)) {
          pred_1_target <- predict(model_1, newdata = target_population, type = prediction_type)
          pred_0_target <- predict(model_0, newdata = target_population, type = prediction_type)
      ...
      # Fallback to gaussian if other families fail
      if (outcome_family != "gaussian") {
        cat("⚠️ Family", outcome_family, "failed, using gaussian\n")
        model_1 <- glm(formula_obj, data = treated, family = "gaussian")
        model_0 <- glm(formula_obj, data = control, family = "gaussian")
...
if (!exists("double_ml_stc_analysis")) {
  double_ml_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols, 
    ...
    tryCatch({
      model <- glm(formula_obj, data = data, family = outcome_family)
      ...
      result <- list(
        ...
        target_predictions = if (!is.null(target_population)) {
          ...
          pred_1 <- predict(model, newdata = target_with_treatment, type = prediction_type)
          pred_0 <- predict(model, newdata = target_without_treatment, type = prediction_type)
      ...
      # Fallback to gaussian if other family fails
      if (outcome_family != "gaussian") {
        cat("⚠️ Double ML: Family", outcome_family, "failed, using gaussian\n")
        ...
        stop("Double ML GLM fitting failed: ", e$message)
      }
    })
  }
}
...
if (!exists("aipw_stc_analysis")) {
  aipw_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols, 
    ...
    # Propensity score model (always binomial for treatment)
    ...
    tryCatch({
      outcome_model_1 <- glm(outcome_formula, data = treated_data, family = outcome_family)
      outcome_model_0 <- glm(outcome_formula, data = control_data, family = outcome_family)
      
      # Predictions
      mu_1 <- predict(outcome_model_1, newdata = data, type = prediction_type)
      mu_0 <- predict(outcome_model_0, newdata = data, type = prediction_type)
      ...
      result <- list(
        ...
        ci_upper = ate_estimate + 1.96 * ate_se,
        target_predictions = if (!is.null(target_population)) {
          mu_1_target <- predict(outcome_model_1, newdata = target_population, type = prediction_type)
          mu_0_target <- predict(outcome_model_0, newdata = target_population, type = prediction_type)
      ...
      # Fallback to gaussian if other family fails
      if (outcome_family != "gaussian") {
        cat("⚠️ AIPW: Family", outcome_family, "failed, using gaussian\n")
        outcome_model_1 <- glm(outcome_formula, data = treated_data, family = "gaussian")
        outcome_model_0 <- glm(outcome_formula, data = control_data, family = "gaussian")
        ...
        stop("AIPW GLM fitting failed: ", e$message)
      }
    })
  }
}
...
if (!exists("causal_forest_stc_analysis")) {
  causal_forest_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols, 
    ...
    # Check for grf package
    if (requireNamespace("grf", quietly = TRUE)) {
      # If grf is available, we could implement actual Causal Forest here
      # For now, we'll fall through to Random Forest fallback
      cat("⚠️ grf available but using Random Forest fallback for this demo\n")
    } else {
      cat("⚠️ grf package not available, using Random Forest T-learner fallback\n")
    }
    
    # Random Forest T-learner fallback implementation
    if (requireNamespace("randomForest", quietly = TRUE)) {
      ...
      # Convert outcome to factor for classification if binary
      outcome_family <- detect_outcome_family(data[[outcome_col]])
      
      treated_y <- if (outcome_family == "binomial") {
        as.factor(treated[[outcome_col]])
      } else {
        treated[[outcome_col]]
      }
      
      control_y <- if (outcome_family == "binomial") {
        as.factor(control[[outcome_col]])
      } else {
        control[[outcome_col]]
      }
      ...
      if (outcome_family == "binomial") {
        # For binary outcomes, get class probabilities
        pred_1 <- predict(rf_1, newdata = data[, X_cols, drop = FALSE], type = "prob")[, "1"]
        pred_0 <- predict(rf_0, newdata = data[, X_cols, drop = FALSE], type = "prob")[, "1"]
      } else {
        # For continuous outcomes, get direct predictions
      ...
      result <- list(
        ...
        target_predictions = if (!is.null(target_population)) {
          if (outcome_family == "binomial") {
            pred_1_target <- predict(rf_1, newdata = target_population[, X_cols, drop = FALSE], type = "prob")[, "1"]
            pred_0_target <- predict(rf_0, newdata = target_population[, X_cols, drop = FALSE], type = "prob")[, "1"]
          } else {
...
if (!exists("r_learner_stc_analysis")) {
  r_learner_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols,
    ...
    tryCatch({
      # Fit models (treatment is always binomial, outcome depends on data)
      m_model <- glm(outcome_formula, data = data, family = outcome_family)
      e_model <- glm(treatment_formula, data = data, family = "binomial")
      
      # Get residuals
      Y_tilde <- data[[outcome_col]] - predict(m_model, type = prediction_type)
      ...
      # Fallback to gaussian for outcome model if other family fails
      if (outcome_family != "gaussian") {
        cat("⚠️ R-learner: Family", outcome_family, "failed, using gaussian\n")
        m_model <- glm(outcome_formula, data = data, family = "gaussian")
        e_model <- glm(treatment_formula, data = data, family = "binomial")
        ...
        stop("R-learner GLM fitting failed: ", e$message)
      }
    })
  }
}
...
if (!exists("bart_stc_analysis")) {
  bart_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols,
    ...
    # Check for BART package availability
    if (requireNamespace("BART", quietly = TRUE)) {
      # If BART is available, we could implement actual BART analysis here
      # For now, we'll use T-learner fallback for this demo
      cat("⚠️ BART available but using T-learner fallback for this demo\n")
    } else {
      cat("⚠️ BART package not available, using T-learner fallback\n")
    }
    
    # Use T-learner fallback implementation
    return(t_learner_stc_analysis(data, outcome_col, treatment_col, covariate_cols, 
                                  "glm", target_population, seed))
  }
}
...
if (!exists("tmle_stc_analysis")) {
  tmle_stc_analysis <- function(data, outcome_col, treatment_col, covariate_cols,
    ...
    # Check for TMLE package availability
    if (requireNamespace("SuperLearner", quietly = TRUE) && 
        requireNamespace("tmle", quietly = TRUE)) {
      # If TMLE packages are available, we could implement actual TMLE here
      # For now, we'll use AIPW fallback for this demo
      cat("⚠️ TMLE packages available but using AIPW fallback for this demo\n")
    } else {
      cat("⚠️ SuperLearner or tmle package not available, using AIPW fallback\n")
    }
    
    # Use AIPW fallback implementation
