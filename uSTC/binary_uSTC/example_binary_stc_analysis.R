################################################################################
##################### Binary Unanchored STC Analysis Example #################
################################################################################
# 
...
# Date: Created automatically from example_binary_stc_analysis.Rmd
# 
# This example demonstrates the Binary Unanchored STC Analysis using the TEM 
...
# focuses on binary outcomes for unanchored simulated treatment comparison.
################################################################################
...
generate_binary_outcomes <- function(data) {
  ...
  # Outcome 2: Safety Endpoint (Any Grade ≥3 Adverse Event)
...
mapping_df <- data.frame(
  ...
  Description = c("Sex (1=Male, 0=Female)", 
                 ...
                 "High BMI ≥30 (1=Yes, 0=No)",
                 "High comorbidity ≥3 (1=Yes, 0=No)")
)
...
tryCatch({
  ...
  cat("⚠️  Results saved to temp_analysis_results.rds\n")
...
cat("🔬 STC CALCULATION METHOD (User's Approach):\n\n")

cat("1. **Probability Calculation:**\n")
cat("   • Treatment probability: exp(coef) / (1 + exp(coef))\n") 
cat("   • Comparator probability: From baseline data\n\n")

cat("2. **Variance Calculation:**\n")
cat("   • Variance from CI width: ((upper - lower) / (2*qnorm(0.975)))^2\n")
cat("   • No reliance on binom.test or other methods\n\n")

cat("3. **Relative Risk Calculation:**\n")
cat("   • RR = Prob_treatment / Prob_comparator\n")
cat("   • SE_log_RR = sqrt(var_treatment/prob_treatment^2 + var_comparator/prob_comparator^2)\n")
cat("   • CI: exp(log(RR) ± 1.96 * SE_log_RR)\n\n")

cat("4. **Bootstrap Enhancement:**\n")
cat("   • 1000 bootstrap samples with replacement\n")
cat("   • Percentile confidence intervals (2.5%, 97.5%)\n")
cat("   • Robust validation of point estimates\n\n")
...
if (length(results$bootstrap_results) > 0) {
  ...
  cat("📊 BASE-CASE MODEL COMPARISON:\n")
...
cat("BINARY STC ANALYSIS COMPLETED\n")
cat("Report Location:", if(exists("report_file")) report_file else "reports/Oncology_Trial_analysis_report.html", "\n")
